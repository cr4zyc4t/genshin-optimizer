...
export class GoogleDriveApiClient {
  ...
  public async findFile(
    ...
    const url = `${this.driveApiBase}/files?spaces=appDataFolder&q=${q}&fields=${fields}&orderBy=modifiedTime%20desc`
  ...
  public async createFile<T = unknown>(
    ...
    const url = `${this.uploadApiBase}/files?uploadType=multipart&fields=${fields}`
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  public async updateFile<T = unknown>(
    ...
    const fields = encodeURIComponent('id,name,modifiedTime,size,version')
    const url = `${this.uploadApiBase}/files/${fileId}?uploadType=media&fields=${fields}`
    const res = await fetch(url, {
      method: 'PATCH',
      headers: {
        Authorization: `Bearer ${accessToken}`,
